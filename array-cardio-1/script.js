...
const filterResults = inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600);
...
// const de = links.map(link => link.textContent).filter(city => city.includes('de'));
// console.table(de);

// 7. sort Exercise
// Sort the people alphabetically by last name
const sortedPeople = people.map(last => last.split(',')[1]).sort();
...
const autoCount = data.reduce((auto, item) => {
  if (!auto[item]) {
    auto[item] = 0;
  }
  auto[item]++;
